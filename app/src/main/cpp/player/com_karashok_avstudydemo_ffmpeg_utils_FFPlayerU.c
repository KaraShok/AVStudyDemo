//
// Created by KaraShokZ on 2021/12/12.
//
...
void player_wait_for_frame(Player *player, int64_t stream_time,
    ...
    for(;;){
        ...
        //等待指定时长
//        int timeout_ret = pthread_cond_timedwait(&player->cond,&player->mutex, sleep_time/1000ll);
...
void jni_audio_prepare(JNIEnv *env,jclass jcls,Player *player){
    //JNI begin------------------

    //AudioTrack对象
    jmethodID create_audio_track_mid = (*env)->GetStaticMethodID(env,jcls,"createAudioTrack","(II)Landroid/media/AudioTrack;");
    jobject audio_track = (*env)->CallStaticObjectMethod(env,jcls,create_audio_track_mid,player->out_sample_rate,player->out_channel_nb);
...
JNIEXPORT void JNICALL Java_com_karashok_avstudydemo_ffmpeg_1utils_FFPlayerU_play
    ...
    jni_audio_prepare(env,jcls,player);
